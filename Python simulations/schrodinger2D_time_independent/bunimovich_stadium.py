...
def schrodinger2D(
    ...
    np.savetxt("bun_eigs.txt", eigenvalue)
...
def eval_wavefunctions(
    ...
    np.savetxt("eigenvalues_bunimovich.txt", evl)
...
def stadium_wavefunctions_plot(R, L, V0, neigs, E0):
    # R=stadium radius
    # L=stadium length
...
stadium_wavefunctions_plot(1, 2, 1e6, 2000, 100)


def spacing_predictions():
    eigen = np.loadtxt(("eigenvalues_bunimovich.txt"), dtype=str)
    eingenfloat = [eigen[i].astype(complex) for i in range(len(eigen))]
    n_evl = len(eingenfloat)
    eigenv = np.array(eingenfloat)
    eigenv = eigenv.real
    eigenv = np.sort(eigenv)
    # print(eigenv)
    spacing = []
    for e in range(1, n_evl - 1):
        spacing.append(max(eigenv[e + 1] - eigenv[e], eigenv[e] - eigenv[e - 1]))
    return spacing


def PGOE(s1, s2):
    a = 27 / (8 * np.pi)
    e = -(2 / 3) * a * (s1 ** 2 + s1 * s2 + s2 ** 2)
    Pgoe = 4 * np.sqrt(2 / (np.pi * 3)) * a ** (5 / 2) * s1 * s2 * (s1 + s2) * np.exp(e)
    return Pgoe


spacing = np.array(spacing_predictions())

print(f"Total time: {np.abs(round(start-time.time()))} seconds")

bins = int(1 + 3.332 * np.log(len(spacing)))
plt.hist(spacing, bins, density=True, histtype="step", fill=False, color="b")
plt.show()
