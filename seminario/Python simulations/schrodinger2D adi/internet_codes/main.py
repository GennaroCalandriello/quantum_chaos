...
def init():
    ...
    HX = 1 * sp.sparse.identity(N * N) - 1j * (delta_t / 2) * (
        LAPLACE_MATRIX - V_x_matrix
    )
    HX = sp.sparse.dia_matrix(HX)

    HY = 1 * sp.sparse.identity(N * N) - 1j * (delta_t / 2) * (
        LAPLACE_MATRIX - V_y_matrix
    )
            ...
            if field.isObstacle(x_axis[j], y_axis[i]):
                ...
                for xx, yy in adj:
                    if (
                        ...
                        and xx < N
                        and yy < N
                        and not field.isObstacle(x_axis[yy], y_axis[xx])
                    ):
...
def plot_animation(t):
    ...
    plt.imshow(
        rgb_map, interpolation="none", extent=[-size / 2, size / 2, -size / 2, size / 2]
    )
...
if len(sys.argv) >= 2:
    ...
    ani = animation.FuncAnimation(
        fig, plot_animation, frames=frame, blit=False, interval=0, repeat=False
    )
