...
/* for billiard shape D_CIRCLES */
{
    ...
    case (C_HEX):
    {
        ...
        for (i = 0; i < NGRIDX; i++)
            for (j = 0; j < NGRIDY + 1; j++)
            {
                ...
                if ((circley[n] < YMAX + MU) && (circley[n] > YMIN - MU))
                    circleactive[n] = 1;
                else
                    circleactive[n] = 0;
            }
    ...
    case (C_GOLDEN_SPIRAL):
    {
        ...
        for (i = 0; i < ncircles; i++)
        {
            ...
            if ((circley[i] < YMAX + MU) && (circley[i] > YMIN - MU))
                circleactive[i] = 1;
...
double x, y;
{
    ...
    case (D_YOUNG):
    {
        if ((x < -MU) || (x > MU))
            return (1);
        else if ((vabs(y - LAMBDA) < MU) || (vabs(y + LAMBDA) < MU))
            return (1);
        else
            return (0);
    }
    ...
    case (D_MENGER):
    {
        ...
        for (k = 0; k < MDEPTH; k++)
        {
            x1 = x1 * (double)MRATIO;
            y1 = y1 * (double)MRATIO;
            if ((vabs(x) < 1.0) && (vabs(y) < 1.0) && (((int)x1 % MRATIO) == MRATIO / 2) && (((int)y1 % MRATIO) == MRATIO / 2))
                return (0);
        }
        return (1);
    }
    ...
    case (D_MENGER_HEATED):
    {
        ...
        else
        {
            ...
            for (k = 0; k < MDEPTH; k++)
            {
                x1 = x1 * (double)MRATIO;
                y1 = y1 * (double)MRATIO;
                if ((((int)x1 % MRATIO) == MRATIO / 2) && (((int)y1 % MRATIO) == MRATIO / 2))
                    return (k + 2);
            }
            return (1);
        }
    ...
    case (D_MANDELBROT):
    {
        ...
        if (u * u + v * v < MANDELLIMIT)
            return (0);
        else if ((x - 0.5) * (x - 0.5) / 3.0 + y * y / 1.0 > 1.2)
            return (2);
        else
            return (1);
    }
    case (D_MANDELBROT_CIRCLE):
    {
        ...
        if (u * u + v * v < MANDELLIMIT)
            return (0);
        else if ((x - LAMBDA) * (x - LAMBDA) + (y - 0.5) * (y - 0.5) < MU * MU)
            return (2);
        else
            return (1);
    }
...
void draw_billiard() /* draws the billiard boundary */
{
    ...
    case (D_CIRCLES):
    {
        ...
        for (i = 0; i < ncircles; i++)
            if (circleactive[i])
                draw_circle(circlex[i], circley[i], circlerad[i], NSEG);
        break;
    }
    case (D_MENGER):
    {
